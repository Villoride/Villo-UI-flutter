...
class _RideBookingPageState extends State<RideBookingPage> {
  ...
  static const double perKm = 12; // INR per km example
  ...
  Future<void> _setDefaultPickupToCurrent() async {
    try {
      ...
      setState(() {
        ...
        _markers.add(Marker(markerId: const MarkerId('pickup'), position: latLng, icon: BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueGreen)));
      });
  ...
  Future<void> _openSearch(bool isPickup) async {
    LatLngHint? bias;
    if (_pickupLatLng != null) bias = LatLngHint(_pickupLatLng!.latitude, _pickupLatLng!.longitude);
    final result = await Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => SearchLocationPage(locationBias: bias)),
    );
    if (result is PlaceDetailsResult) {
      setState(() {
        if (isPickup) {
          ...
          _markers.add(Marker(markerId: const MarkerId('pickup'), position: _pickupLatLng!, icon: BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueGreen)));
        } else {
          _dropLatLng = LatLng(result.lat, result.lng);
          _dropAddress = result.address;
          _markers.removeWhere((m) => m.markerId.value == 'drop');
          _markers.add(Marker(markerId: const MarkerId('drop'), position: _dropLatLng!, icon: BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueRed)));
        }
      });
      ...
      if (_pickupLatLng != null && _dropLatLng != null) {
        ...
        c.animateCamera(CameraUpdate.newLatLngZoom(_pickupLatLng!, 15));
      } else {
        final c = await _controller.future;
        c.animateCamera(CameraUpdate.newLatLngZoom(_dropLatLng!, 15));
      }
    }
  }
  ...
  Future<void> _drawRouteAndCalculate() async {
    ...
    final distanceText = leg["distance"]["text"]; // e.g., "5.6 km"
    final distanceValMeters = leg["distance"]["value"] * 1.0; // meters
  ...
  // simulate nearby drivers (random positions near pickup)
  void _startSimulateDrivers() {
    if (_pickupLatLng == null) return;
    _driverMarkers.clear();
    final rand = Random();
    for (int i = 0; i < 6; i++) {
      final dLat = _pickupLatLng!.latitude + (rand.nextDouble() - 0.5) / 500; // ~±0.001
  ...
  Future<void> _confirmBooking() async {
    ...
    // fake "search for driver" delay
    await Future.delayed(const Duration(seconds: 4));

    // For demo: pick the nearest simulated driver
    String? assignedDriverId;
    double? bestDist;
    for (var e in _driverMarkers.entries) {
      final d = distanceBetween(_pickupLatLng!.latitude, _pickupLatLng!.longitude, e.value.latitude, e.value.longitude);
    ...
    // Build booking data
    final booking = {
      "pickup": {"lat": _pickupLatLng!.latitude, "lng": _pickupLatLng!.longitude, "address": _pickupAddress},
      "drop": {"lat": _dropLatLng!.latitude, "lng": _dropLatLng!.longitude, "address": _dropAddress},
    ...
    // POST to backend — replace URL with your API
    try {
      ...
      if (res.statusCode == 200 || res.statusCode == 201) {
        ...
        // For demo, treat as success if backend not available
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Driver assigned (demo).")));
      }
    } catch (e) {
      // network/backend error: still continue demo
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Booking saved locally (demo).")));
    } finally {
  ...
  Widget build(BuildContext context) {
    Set<Marker> driverMarkerSet = _driverMarkers.entries.map((e) {
      return Marker(
    ...
    final polylines = <Polyline>{
      if (_polylineCoords.isNotEmpty)
        Polyline(polylineId: const PolylineId("route"), color: Colors.blueAccent, points: _polylineCoords, width: 5),
    };

    return Scaffold(
      appBar: AppBar(title: const Text("Villo Ride"), backgroundColor: Colors.black87),
      body: Stack(
        children: [
          GoogleMap(
            initialCameraPosition: CameraPosition(target: _pickupLatLng ?? const LatLng(20.5937, 78.9629), zoom: 5),
          ...
          Positioned(
            ...
            child: Column(
              children: [
                _topCard(icon: Icons.my_location, title: _pickupAddress ?? "Pickup", subtitle: _pickupLatLng == null ? "Not set" : "", onTap: () => _openSearch(true)),
                const SizedBox(height: 8),
                _topCard(icon: Icons.place, title: _dropAddress ?? "Drop", subtitle: _dropLatLng == null ? "Tap to set drop" : "", onTap: () => _openSearch(false)),
              ],
            ),
          ),
              ...
              children: [
                    ...
                    child: Row(
                      ...
                      children: [
                        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Text(_distanceKm != null ? "${_distanceKm!.toStringAsFixed(2)} km" : "---", style: const TextStyle(fontWeight: FontWeight.bold)),
                          const SizedBox(height: 6),
                          Text(_etaText ?? "--"),
                        ]),
                        Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                          Text(_fare != null ? "₹ ${_fare!.toStringAsFixed(0)}" : "--", style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                          const SizedBox(height: 6),
                          Text("Est. fare", style: TextStyle(color: Colors.grey[600])),
                        ]),
                      ],
                    ),
                  ),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: (_pickupLatLng != null && _dropLatLng != null && !_searchingDriver) ? _confirmBooking : null,
                    style: ElevatedButton.styleFrom(backgroundColor: Colors.blueAccent, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
                    child: _searchingDriver ? const Text("Searching driver...") : const Text("Confirm & Book"),
                  ),
                )
              ],
            ),
  ...
  Widget _topCard({required IconData icon, required String title, String? subtitle, required VoidCallback onTap}) {
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(fontWeight: FontWeight.bold)), if (subtitle != null && subtitle.isNotEmpty) Text(subtitle, style: const TextStyle(color: Colors.black54))])),
